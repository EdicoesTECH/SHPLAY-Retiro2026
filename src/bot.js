...
async function generateBadge(photoDataUrl, userName) {
  ...
  drawSpacedText(ctx, 'RETIRO QUARESMAL ONLINE', width / 2, 38, 3)

  ctx.font = 'italic 600 22px Cormorant Garamond, Georgia, serif'
  ctx.fillStyle = '#f5eedd'
  ctx.fillText(eventName, width / 2, 68)
  ...
  drawSpacedText(ctx, 'PARTICIPANTE', width / 2, photoY + photoSize - 60, 2)

  fitText(ctx, userName, {
    maxWidth: photoSize - 40,
    initialSize: 38,
    minSize: 22,
    weight: '700',
  ...
  drawSpacedText(ctx, '✦  27 E 28 DE MARÇO  ✦', width / 2, footerY + 28, 2)

  ctx.fillStyle = 'rgba(245,238,221,0.4)'
  ctx.font = 'italic 300 13px Cormorant Garamond, Georgia, serif'
  ctx.fillText(eventSubtitle || 'Online e ao Vivo', width / 2, footerY + 50)
...
function drawSpacedText(ctx, text, x, y, spacing = 0) {
  ...
  chars.forEach((ch, index) => {
    ctx.fillText(ch, currentX + widths[index] / 2, y)
